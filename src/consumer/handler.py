...
class MessageHandler:
    ...
    def is_valid(self, payload: CloudEvent) -> bool:
        if not (set(["url", "time_in_ms", "http_status_code"]
                    ).issubset(list(payload.data.keys()))):
            logging.debug(
                '[MessageHandler.is_valid] Received data is invalid - {}'.format(payload.data.keys()))
            return False

        if not ("time" in payload):
            logging.debug(
                '[MessageHandler.is_valid] Missing time in cloudevents obj - {}'.format(payload))
            return False
        ...
        if not (
            ...
            logging.debug(
                '[MessageHandler.is_valid] Received message is invalid - {}'.format(payload))
            return False
    ...
    def handle(self, message):
        ...
        logging.info(
            '[MessageHandler.handle] - Incoming message - url: {}'.format(payload.data["url"]))
